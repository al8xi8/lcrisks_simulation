...
stage <- function(prob){
  LCStage = 0
  p_rand2 <- runif(1)
  if(p_rand2<prob[1]){LCStage = 1}
  else if(p_rand2<sum(prob[1:2])){LCStage = 2}
  else if(p_rand2<sum(prob[1:3])){LCStage = 3}
  else if(p_rand2<sum(prob[1:4])){LCStage = 4}
  else if(p_rand2<sum(prob[1:5])){LCStage = 5}
  else{LCStage = 6}
  return(LCStage)
}
histology <- function(sex, prshist, fmhist, copd, currsmk, fmrsmk, bmi, cpd, currsmkdur_atdiag, curryearsquit_atdiag){
